...
**两期限界影响复核**：把两期测点序列分别作为不闭合折线复核净距，给出净距变化与是否由合格转为不合格；
以及**设计控制点对激光测量轨迹的覆盖复核**：逐点给出控制点到测量折线的最短距离、最近线段与是否覆盖，
避免断面净距计算建立在测量轨迹缺失的区域上。
...
app/
  ...
  coverage.py     # 控制点对测量折线的覆盖分析：点到折线最短距离与覆盖判定（纯 Python）
...
- `verify` 是**一次性验收服务**：等待 `api` 健康检查通过后，对其执行端到端断言
  ...
  控制点全部覆盖 / 首个遗漏仍返回完整结果 / 并列线段选择稳定 / 无效折线拒绝），
...
### `POST /api/profiles/coverage`

控制点覆盖复核：隧道激光采集结束后，复核人员确认设计指定的**拱顶、侧墙和设备邻近控制点**
均被按序连接的测量折线（测量轨迹）有效覆盖，避免断面净距计算建立在轨迹缺失区域上。
接口对每个控制点计算到折线各线段的最短欧氏距离并判定是否落在覆盖半径内。

请求体（坐标为**毫米整数**，绝对值不超过 1,000,000）：

| 字段 | 说明 |
| --- | --- |
| `measured_polyline.points` | 激光测量轨迹点，**按顺序连接但不闭合**，至少 2 个点，相邻点不得重合（规则同 `tunnel_polyline`） |
| `control_points` | 设计控制点列表（**至少 1 个**），每项 `{"name": <str>, "x": <int>, "y": <int>}`，名称非空白且同请求内唯一 |
| `coverage_radius` | 覆盖半径（毫米，**非负整数**）；未舍入最短距离 `<=` 半径判为已覆盖 |

```bash
curl -s http://localhost:8000/api/profiles/coverage \
  -H 'Content-Type: application/json' \
  -d '{
    "measured_polyline": {
      "points": [
...
响应（结果与输入控制点按顺序一一对应；距离 200 / 200 / 0，最近线段起点索引 1 / 0 / 2）：

```json
{
  "points": [
    {"name": "拱顶", "distance_mm": 200.0, "nearest_segment_start_index": 1, "covered": true},
    {"name": "侧墙", "distance_mm": 200.0, "nearest_segment_start_index": 0, "covered": true},
    {"name": "设备邻近", "distance_mm": 0.0, "nearest_segment_start_index": 2, "covered": true}
  ],
  "first_uncovered_name": null,
  "all_covered": true
}
```
...
- `points[].distance_mm`：控制点到测量折线的最短距离（点到线段垂足落在线段外时取到较近端点距离），
  四舍五入到三位小数；
- `points[].nearest_segment_start_index`：最近线段的起点在折线点序列中的下标。
  **两条线段距离之差不超过 `1e-9` 视为并列，取起点索引较小者**；
- `points[].covered`：**未舍入**最短距离 `<= coverage_radius` 时为 `true`
  （点恰好落在半径边界上也算覆盖；半径 0 时仅轨迹上的点算覆盖）；
- `first_uncovered_name`：首个未覆盖控制点的名称；全部覆盖时为 `null`。
  **单个控制点未覆盖不中断批次**，后续控制点仍完成计算，响应始终包含完整结果。

覆盖复核特有的 `422` 校验（复用既有错误信封，**任何校验失败都不生成部分报告**，错误定位到具体字段）：

- 测量折线少于 2 个点：`too_few_points`，定位到 `measured_polyline.points`；
- 测量折线含相邻重合点：`duplicate_adjacent_point`，定位到 `measured_polyline.points.<下标>`；
- 控制点名称仅含空白 / 控制字符：`blank_control_point_name`，定位到 `control_points.<下标>.name`；
- 控制点名称重复：`duplicate_control_point_name`，定位到重复出现的后者；
- 控制点列表为空：`too_short`，定位到 `control_points`；
- 覆盖半径为负 / 非整数 / 布尔冒充：`greater_than_equal` / `int_type`，定位到 `coverage_radius`；
- 坐标非整数 / 越界、多余字段等同既有规则；多个问题一次性返回。
...
- 控制点覆盖：点到不闭合折线的最短距离取各线段（复用点到线段距离基础能力）距离的最小值；
  并列规则同上（距离差不超过 `1e-9` 取起点索引较小者）；覆盖门槛用**未舍入**距离与半径比较，
  恰好等于半径判覆盖，只有输出距离四舍五入到三位小数。
...
控制点覆盖复核（服务层点到折线距离与并列选择 / 全部覆盖 / 首个遗漏不中断仍返回完整结果 /
未舍入半径边界 / 并列线段稳定取较小起点索引 / 折线过短与相邻重合拒绝 /
空白重名空列表与非法半径拒绝 / 不生成部分报告），
